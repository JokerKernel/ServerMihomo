...
可选环境变量：
  ...
  SERVERMIHOMO_INSTALL_DIR   安装目录，默认为 /usr/local/sbin
  SERVERMIHOMO_BINARY_NAME   安装后的命令名，默认为 mihomo
EOF
}
...
ASSET="snailproxy-linux-${ARCH}-${RELEASE_VERSION}"
...
DOWNLOADED_RELEASE="$(printf '%s\n' "$DOWNLOADED_VERSION" | awk '$1 == "snailproxy" { print $2; exit }')"
[ "$DOWNLOADED_RELEASE" = "$RELEASE_VERSION" ] || fail "下载文件的版本不匹配：期望 ${RELEASE_VERSION}，实际 ${DOWNLOADED_VERSION:-未知}"
